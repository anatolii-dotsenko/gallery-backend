...
connectDB().catch((err: any) => console.error("Помилка підключення до MongoDB:", err));
...
app.post("/api/images", upload.single("image"), async (req: Request, res: Response): Promise<void> => {
  if (!req.file) { 
...
app.get("/api/images/:id", async (req: Request<{ id: string }>, res: Response) => {
  try {
    const bucket = getBucket();
    // Використовуємо mongoose.mongo.ObjectId
    bucket.openDownloadStream(new mongoose.mongo.ObjectId(req.params.id)).pipe(res);
  } catch (error) {
...
app.delete("/api/images/:id", async (req: Request<{ id: string }>, res: Response) => {
  try {
